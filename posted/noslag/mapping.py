"""Variable mappings for harmonising TEDFs.

Variable mappings can be defined as subclasses to AbstractVariableMapper and
AbstractVariableGroupMapper and can be called in the select step of the
NOSLAG processing of a TEDF.
"""
...
class AbstractVariableMapper(ABC):
    """Abstract class for defining variable mappings.

    Mappings defined with this class are applied to all rows equally.
    """
    ...
    def raise_warnings(self, selected: pd.DataFrame) -> None:
        """Raise warnings collected during mapping."""
        for warn_id, warn_locs in self._warnings.items():
            locs = pd.concat(warn_locs)
            rows = selected.loc[locs.reindex(selected.index, fill_value=False)]
    ...
    def map(self) -> pd.DataFrame:
        ...
        Applies the mapping to the defined dataframe.
        """
...
class AbstractVariableGroupMapper(AbstractVariableMapper):
    """Subclass of AbstractVariableMapper for group-wise operations.

    Unlike AbstractVariableMapper, mappings defined with
    AbstractVariableGroupMapper will be applied group-wise, i.e. for
...
def _map_variables(
    ...
    # GroupBy once.
    if fields:
        groups = selected.groupby(fields, sort=False).indices.values()
    else:
    ...
    for mapping in mappings:
        ...
        for attribute_name in dir(module):
            ...
            if (
                ...
                and issubclass(attribute, AbstractVariableMapper)
                and attribute != AbstractVariableMapper
                and attribute != AbstractVariableGroupMapper
            ):
