"""Express activitiy-type variables relative to reference."""
...
class ActivitiesMapper(AbstractVariableGroupMapper):
    ...
    def _prepare_units(self) -> None:
        if not (
            self._cond_capacity_change | self._cond_tot_capacity_change
        ).any():
            return
        ...
        if self._reference_capacity not in self._units:
            self._units[self._reference_capacity] = (
                self._units[self._ref_cap_activity] + "/year"
            )

        self._conv_factor_cap = (
            (Q(self._units[self._ref_cap_activity] + "/year"))
            .to(self._units[self._reference_capacity])
            .m
        )
    ...
    def _map(self, group: pd.DataFrame, cond_group: pd.Series) -> pd.DataFrame:
        ...
        matrix.values[
            np.arange(len(df)), matrix.columns.get_indexer(df["variable"])
        ] = -1.0
        matrix.values[
            np.arange(len(df)),
            matrix.columns.get_indexer(df["reference_variable"]),
        ] = df["value"].values

        # Add last row and inhomogeneity, which enforce that the reference
        # activity equals to 1.0.
        ...
        try:
            ...
            group.loc[cond_activity, "variable"] = df["variable"].where(
                df["variable"] != self._reference_activity,
                df["reference_variable"],
            )
            group.loc[cond_activity, "reference_variable"] = (
                self._reference_activity
            )
            group.loc[cond_activity, "value"] = harmonised_activities[
                matrix.columns.get_indexer(
                    group.loc[cond_activity, "variable"]
                )
            ]

        if cond_capacity_change.any() or cond_tot_capacity_change.any():
            if (
                harmonised_activities is None
                or self._ref_cap_activity != self._reference_activity
            ):
                last_row = pd.Series(0.0, index=cols)
            ...
            if cond_capacity_change.any():
                cap_vars = group.loc[
                    cond_capacity_change, "reference_variable"
                ].rename("to")
                act_vars = cap_vars.str.replace(
                    r"(Input|Output) Capacity", r"\1", regex=True
                ).rename("from")
                a = harmonised_activities[matrix.columns.get_indexer(act_vars)]
                b = pd.concat([act_vars, cap_vars], axis=1).apply(
                    lambda row: (
                        Q(self._units[row["from"]] + "/year")
            ...
            if cond_tot_capacity_change.any():
                cap_vars = group.loc[
                    cond_tot_capacity_change, "variable"
                ].rename("to")
                act_vars = cap_vars.str.replace(
